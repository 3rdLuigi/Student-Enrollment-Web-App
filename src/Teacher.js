...
import { loadClassesFromStorage, saveClassesToStorage, loadTeacherClasses, saveTeacherClasses } from './utils';
import './Teacher.css';
...
function Teacher() {
    ...
    return (
        <div>
            ...
            {!loggedIn ? (
                <button onClick={() => setLoggedIn(true)} className="login-button">Log In as Teacher</button>
            ) : (
                <div>
                    <button onClick={() => { setLoggedIn(false); setClasses([]); setLoading(true); }} className="logout-button">Log Out</button>
                    <h2>My Classes</h2>
